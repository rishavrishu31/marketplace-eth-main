  ...
  async function catchRevert(promise) {
    try {
      ...
      const revertFound =
        ...
        error.message.search("Returned error: VM Exception while processing transaction: revert") >= 0;
...
describe("Normal withdraw", () => {
  ...
  it("should fail when withdrawing OVER limit balance", async () => {
    await catchRevert(_contract.withdraw(overLimitFunds, { from: currentOwner }), "Withdrawal amount exceeds contract balance.")
  })
